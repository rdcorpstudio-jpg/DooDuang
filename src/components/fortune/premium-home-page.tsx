 ...
 * Bottom-nav พรีเมียม:
 * locked → sales
 * unlocked → same daily UI as free result, but fully unlocked (replaces old TOC report)
 */
export function PremiumHomePage({
  ...
  useEffect(() => {
    ...
    setUnlocked(
      forceUnlocked ||
        isPremiumUnlocked(
          next
            ? { birthDate: next.birthDate, nickname: next.nickname }
            : null
        )
    );
    setReady(true);
  ...
  const seed = useMemo(() => {
    ...
    return `overall-${profile.realName}-${profile.nickname}-${profile.birthDate}-${profile.gender}`;
  }, [previewSeed, profile]);
